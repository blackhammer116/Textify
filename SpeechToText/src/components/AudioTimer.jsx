...
const AudioTimer = ({ isRunning,
    ...
    return (
        <div className=" text-[25px] text[#fff] mt-4 font-semibold " >
            <div className="time">
                {hours}:{minutes.toString().padStart(2, "0")}:
                <span className=" w-[23px]  text[#fff] inline-block "> {seconds.toString().padStart(2, "0")}:</span>
                <span className=" w-[23px] text[#fff] inline-block ml-3" >{milliseconds.toString().padStart(2, "0")}</span>
            </div>
        </div>
    );
};
